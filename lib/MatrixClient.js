...
class MatrixClient extends events_1.EventEmitter {
    ...
    async getRoomMessages(roomId, opts = {}) {
        ...
        const res = await this.doRequest("GET", `/_matrix/client/v3/rooms/${encodeURIComponent(roomId)}/messages`, qs);
        ...
        if (isEncrypted && this.crypto) {
            ...
            for (const event of res.chunk) {
                if (event.type === "m.room.encrypted") {
                    try {
                        const decrypted = await this.crypto.decryptRoomEvent(new EncryptedRoomEvent_1.EncryptedRoomEvent(event), roomId);
...
__decorate([
    ...
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], MatrixClient.prototype, "getRoomMessages", null);
__decorate([
    (0, decorators_1.timedMatrixClientFunctionCall)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
